...
function mostrarbarcos(){
  ...
  if (matrizba.length > 0) {
    if (
      matrizba[matrizba.length - 1] === undefined ||
      matrizba[matrizba.length - 1].body.position.x < width - 300
    ) {
      var positions = [-40, -60, -70, -20];
      var position = random(positions);
      var barc = new barco(width, height - 100, 170, 170, position);
    ...
    var barc = new barco(width, height - 60, 170, 170, -60);
    matrizba.push(barc);
  }
